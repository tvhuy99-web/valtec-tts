...
''',
    '''    override fun onResume() {
        super.onResume()
        Diagnostics.log("app", "activity.onResume", snapshot = true)
        saveVoiceProfileButton.isEnabled =
            VoiceProfileStore.activeReference(this)
                ?.let { VoiceProfileStore.hasWarmV4Caches(it) }
                ?: false
    }
''',
    'voice profile state on resume',
)
...
save_helpers = r'''    private fun askSaveVoiceProfile() {
        ...
        AlertDialog.Builder(this)
            .setTitle("Lưu giọng")
            .setMessage("Giọng đã lưu có thể dùng lại mà không cần chọn lại tệp WAV.")
            .setView(input)
        ...
        executor.execute {
            ...
            try {
                ...
                runOnUiThread {
                    ...
                    statusText.text =
                        "Đã lưu giọng “${profile.name}”. Giọng này đã sẵn sàng cho TTS hệ thống."
                }
            } catch (t: Throwable) {
...
if missing:
    raise RuntimeError(f'MainActivity missing early system TTS UI fragments: {missing}')
...
premature = [fragment for fragment in forbidden if fragment in final]
if premature:
    raise RuntimeError(f'MainActivity contains premature final-catalog fragments: {premature}')

print('Connected early system-TTS buttons and saved-voice hooks; final voice catalog is deferred')
