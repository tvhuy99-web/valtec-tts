...
if len(sys.argv) != 3:
    raise SystemExit("usage: patch_vieneu_system_tts_early_audio.py <vieneu-source-dir> <android-root>")
...
def regex_once(path: pathlib.Path, pattern: str, replacement: str, label: str) -> None:
    text = path.read_text(encoding="utf-8")
    updated, count = re.subn(pattern, lambda _m: replacement, text, count=1, flags=re.DOTALL | re.MULTILINE)
    if count != 1:
...
# ---------------------------------------------------------------------------
# Native engine: expose stable prefix PCM from the SAME utterance.
# This does not split text or alter F32 acoustic sampling. When no callback is
# configured, the branch is inert and synthesis remains byte-for-byte on the
# existing full-decode path except for the extra empty-callback checks.
# ---------------------------------------------------------------------------
replace_once(
    engine_h,
    '#include <memory>\n',
...
# ---------------------------------------------------------------------------
# JNI: allow Kotlin to install a temporary per-request sink. synthesizeDirect
# remains the only synthesis call and still returns the complete FloatArray for
# exact PCM caching; the sink merely receives prefixes while that call runs.
# ---------------------------------------------------------------------------
replace_once(
    native_kt,
...
''',
    '''    external fun synthesizeDirect(text: String, referenceWav: String, voiceId: String, useRefCodes: Boolean, deterministic: Boolean, dialect: String): FloatArray?
    external fun setStreamSink(sink: NativePcmStreamSink?): String
''',
    'Kotlin stream sink JNI declaration',
)
...
std::unique_ptr<VieneuV3NativeEngine> g_engine;
jobject g_stream_sink = nullptr;
jmethodID g_stream_method = nullptr;
''',
    'JNI stream globals',
)
...
Java_com_vieneu_voiceclone_VieNeuNative_setStreamSink(
        JNIEnv* env, jobject, jobject sink) {
    std::lock_guard<std::mutex> stream_lock(g_stream_mutex);
    if (g_stream_sink) {
        env->DeleteGlobalRef(g_stream_sink);
        g_stream_sink = nullptr;
        g_stream_method = nullptr;
    }
    if (!sink) return to_jstring(env, "");

    jclass cls = env->GetObjectClass(sink);
    if (!cls) return to_jstring(env, "Unable to resolve NativePcmStreamSink class.");
    jmethodID method = env->GetMethodID(cls, "onNativePcmChunk", "([FZ)Z");
    env->DeleteLocalRef(cls);
    if (!method) {
        if (env->ExceptionCheck()) env->ExceptionClear();
        return to_jstring(env, "NativePcmStreamSink.onNativePcmChunk([FZ)Z was not found.");
    }
    jobject global = env->NewGlobalRef(sink);
    if (!global) return to_jstring(env, "Unable to retain NativePcmStreamSink.");
    g_stream_sink = global;
    g_stream_method = method;
...
replace_once(
    ...
    stream_setter + 'extern "C" JNIEXPORT jfloatArray JNICALL\nJava_com_vieneu_voiceclone_VieNeuNative_synthesizeDirect(',
    'insert JNI stream sink setter',
)
...
if start < 0 or end < 0:
    raise RuntimeError('Unable to isolate synthesizeDirect for stream callback injection')
...
stream_params = r'''        jobject stream_sink = nullptr;
        jmethodID stream_method = nullptr;
        {
            std::lock_guard<std::mutex> stream_lock(g_stream_mutex);
            stream_sink = g_stream_sink;
            stream_method = g_stream_method;
        }
        if (stream_sink && stream_method) {
...
jni_text = jni_text[:start] + direct + jni_text[end:]
jni.write_text(jni_text, encoding="utf-8")

# ---------------------------------------------------------------------------
# Persist the A/B switch in the same SystemVoiceSettings object.
# Default is OFF so the established full-decode path remains the baseline.
# ---------------------------------------------------------------------------
replace_once(
    store,
    '''    val volume: Float,
...
# ---------------------------------------------------------------------------
# System settings UI: one accessible on/off control for A/B comparison.
# ---------------------------------------------------------------------------
replace_once(
    settings_activity,
...
# ---------------------------------------------------------------------------
# Service: use streaming only on cache misses and only when rate/pitch are 1.0.
# Volume is safe to apply independently per chunk. Cache hits stay exact and
# keep their already-good ~100 ms path.
# ---------------------------------------------------------------------------
...
new_native_call = '''                earlyPlaybackActive = settings.earlyPlayback &&
                ...
                if (streamSink != null) {
                    val streamError = VieNeuNative.setStreamSink(streamSink)
                    if (streamError.isNotEmpty()) {
                        earlyPlaybackActive = false
                        Diagnostics.log(
                            "system_tts",
                            "system_tts.early_playback.fallback",
                            level = "WARN",
                            data = mapOf(
                                "synthesis_id" to synthesisId,
                                "reason" to "native_sink_registration_failed",
                                "error" to streamError,
                            ),
                        )
                    }
                }
                ...
                val direct = try {
                    ...
                    if (streamSink != null) runCatching { VieNeuNative.setStreamSink(null) }
                }
                val nativeDirectMs =
                    (SystemClock.elapsedRealtimeNanos() - nativeStartNs) / 1_000_000.0
                if (direct == null) {
                    if (streamRejected || stopEpoch.get() != epoch ||
                        VieNeuNative.lastError().contains("VIENEU_STREAM_ABORTED", ignoreCase = true) ||
                        VieNeuNative.lastError().contains("VIENEU_CANCELLED", ignoreCase = true)
                    ) {
...
# Do not replay the complete buffer after native already streamed all chunks.
...
# A Binder callback rejection is overwhelmingly a normal TalkBack focus change.
...
# Contracts: fail materialization if any half-wired experimental path slips in.
contracts = {
    ...
    native_kt: ('setStreamSink(sink: NativePcmStreamSink?)',),
    jni: ('Java_com_vieneu_voiceclone_VieNeuNative_setStreamSink', 'onNativePcmChunk', 'params.stream_first_frames = 2'),
    ...
    service: ('system_tts.early_playback.fallback', 'jni_float_stream', 'callback_rejected_as_cancel'),
}
...
print("Applied optional same-utterance early PCM A/B path; baseline remains default OFF")
